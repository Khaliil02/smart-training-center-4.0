...
public class EvaluationService {
        ...
        public EvaluationDto createEvaluation(EvaluationRequest request) {
                Cours cours = coursRepository.findById(request.getCoursId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Cours non trouvé avec l'id: " + request.getCoursId()));
        ...
        @Transactional(readOnly = true)
        public QuizDto getQuizByEvaluationId(Long evaluationId) {
                Evaluation evaluation = findEvaluationOrThrow(evaluationId);
                List<Quiz> quizzes = quizRepository.findByCoursId(evaluation.getCours().getId());
                if (quizzes.isEmpty()) {
                        throw new ResourceNotFoundException(
                                        "Aucun quiz trouve pour l'evaluation avec l'id: " + evaluationId);
                }
                return toQuizDto(quizzes.get(0));
        }
        ...
        public QuizResultDto submitQuiz(QuizSubmissionRequest submission) {
                Quiz quiz = quizRepository.findById(submission.getQuizId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Quiz non trouvé avec l'id: " + submission.getQuizId()));

                Etudiant etudiant = etudiantRepository.findById(submission.getEtudiantId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Étudiant non trouvé avec l'id: " + submission.getEtudiantId()));
                ...
                // Batch-load all answers for all questions at once
                List<Long> questionIds = questions.stream().map(Question::getId).collect(Collectors.toList());
                List<Reponse> allReponses = reponseRepository.findByQuestionIdIn(questionIds);
                Map<Long, List<Reponse>> reponsesByQuestion = allReponses.stream()
                                .collect(Collectors.groupingBy(r -> r.getQuestion().getId()));

                for (Question question : questions) {
                        List<Reponse> questionReponses = reponsesByQuestion.getOrDefault(question.getId(), List.of());
                        List<Long> correctReponseIds = questionReponses.stream()
                ...
                if (passed) {
                        inscriptionCoursRepository.findByEtudiantIdAndCoursId(etudiant.getId(), quiz.getCours().getId())
                                        .ifPresent(inscription -> {
                                                inscription.setProgression(
                                                                Math.max(inscription.getProgression(), percentage));
        ...
        private Evaluation findEvaluationOrThrow(Long id) {
                return evaluationRepository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Évaluation non trouvée avec l'id: " + id));
        }
        ...
        private QuizDto toQuizDto(Quiz q) {
                return QuizDto.builder()
                                ...
                                .questions(q.getQuestions() != null
                                                ? q.getQuestions().stream().map(this::toQuestionDto)
                                                                .collect(Collectors.toList())
                                                : null)
                                .build();
        }
        ...
        private QuestionDto toQuestionDto(Question q) {
                return QuestionDto.builder()
                                ...
                                .reponses(q.getReponses() != null
                                                ? q.getReponses().stream().map(this::toReponseDto)
                                                                .collect(Collectors.toList())
                                                : null)
                                .build();
        }
